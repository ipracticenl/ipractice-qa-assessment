...
public class PsychologistSqlRepository(ApplicationDbContext dbContext) : IPsychologistSqlRepository
{
    ...
    public async Task<Psychologist> GetPsychologistByNameAsync(string name, CancellationToken cancellationToken)
    {
        var psychologist = await dbContext.Psychologists.SingleOrDefaultAsync(x => x.Name == name, cancellationToken);
        return psychologist;
    }
    ...
    public async Task<List<Psychologist>> GetAllPsychologistsAsync(CancellationToken cancellationToken)
    {
        var psychologists = await dbContext.Psychologists.ToListAsync(cancellationToken);
        return psychologists;
    }
